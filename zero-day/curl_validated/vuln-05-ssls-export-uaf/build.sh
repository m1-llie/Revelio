#!/usr/bin/env bash
# Compile and run PoC for vuln-05: Heap-UAF in Curl_ssl_session_export()
# Iterator invalidated when callback calls curl_easy_ssls_import during export
# curl lib/vtls/vtls_scache.c:1214 — requires USE_SSLS_EXPORT (--enable-ssls-export)
# Image: curl-validate:20260417 (curl 8.20.0-DEV, commit 70281e3, ASAN+UBSAN)
set -e

IMAGE="curl-validate:20260417"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
POC_BIN="/tmp/poc-vuln05"

docker run --rm --name curl-val-vuln05 \
