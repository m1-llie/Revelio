#!/usr/bin/env bash
# Compile and run PoC for vuln-02: NULL deref in EVP_PKEY_id()
# SSL_get_privatekey() unchecked (ENGINE/deprecated path) — curl lib/vtls/openssl.c ~1568
# Image: curl-validate:20260417 (curl 8.20.0-DEV, commit 70281e3, ASAN+UBSAN)
set -e

IMAGE="curl-validate:20260417"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
POC_BIN="/tmp/poc-vuln02"

docker run --rm --name curl-val-vuln02 \
