# Signed Integer Overflow in `ImageStream::ImageStream()` for CMYK Images (CWE-190)
...
image XObject with `/ColorSpace /DeviceCMYK` (4 components) and `/Width 536870912`,
the multiplication `536870912 * 4` overflows `int` before the guard has any chance to
execute, making the check permanently ineffective.
...
overflow already corrupts `nVals` at line 596. The check should precede the
multiplication.
...
```bash
docker run --rm \
  -v /scr2/yiwei/vul-agent/zero-day/poppler_validated/01-stream-intovf-cmyk:/work \
...
Signed integer overflow on the `nVals` computation produces a negative or
incorrectly small value. Downstream code uses `nVals` to size buffer allocations
and loop bounds, which can lead to:

- Under-allocation followed by out-of-bounds writes (heap corruption).
- Out-of-bounds reads when iterating pixel data.

Any PDF containing a CMYK image XObject with `/Width` >= `ceil(INT_MAX / 4)` = 536870912
...
Note: Bug 2 (`02-stream-intovf-rgb`) is the same defect triggered via
`/ColorSpace /DeviceRGB` (3 components). The fix above resolves both bugs.
