# Signed Integer Overflow in `ImageStream::ImageStream()` for RGB Images

## Summary

`ImageStream::ImageStream()` in `poppler/Stream.cc` computes `nVals = width * nComps` at line 596 before the bounds check `width > INT_MAX / nComps` at line 598. For a PDF image XObject with `/ColorSpace /DeviceRGB` (3 components) and `/Width 715827883`, the multiplication `715827883 * 3` overflows `int` before the guard executes, making the check permanently ineffective.

This is the same defect as bug <Signed Integer Overflow in `ImageStream::ImageStream()` for CMYK Images>, triggered via RGB (3 components) rather than CMYK (4 components). The fix is identical.
...
The bounds check at line 598 is never reached with a correct result because the overflow already corrupts `nVals` at line 596.
...
Same as bug <Signed Integer Overflow in `ImageStream::ImageStream()` for CMYK Images>. Signed integer overflow on the `nVals` computation produces a negative or incorrectly small value. Downstream code uses `nVals` to size buffer allocations and loop bounds, which can lead to:

- Under-allocation followed by out-of-bounds writes (heap corruption).
- Out-of-bounds reads when iterating pixel data.

Any PDF containing an RGB image XObject with `/Width` >= `ceil(INT_MAX / 3)` = 715827883 can trigger this path. The vulnerability is reachable without authentication whenever poppler is used to render untrusted PDF files.

## Suggested Fix

Move the bounds check to before the multiplication (same fix resolves both this bug and bug <Signed Integer Overflow in `ImageStream::ImageStream()` for CMYK Images>):
