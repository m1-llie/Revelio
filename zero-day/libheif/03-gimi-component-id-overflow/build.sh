#!/usr/bin/env bash
# Build latest libheif with ASAN+UBSan (WITH_UNCOMPRESSED_CODEC=ON) and run SF08 PoC.
# Requires: clang, cmake, make, git (and internet access to clone libheif).
# Usage: bash build.sh
set -e

WORKDIR="/tmp/libheif_sf08_build"
INSTALL_DIR="/tmp/libheif_sf08_inst"
POC_DIR="$(dirname "$0")"
