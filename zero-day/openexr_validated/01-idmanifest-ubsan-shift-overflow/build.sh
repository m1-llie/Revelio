#!/usr/bin/env bash
# Build and run PoC 01: UBSan shift exponent overflow in readVariableLengthInteger
...
docker run --rm \
  ...
  "$IMAGE" bash -c '
    ...
    # ── stub headers required by ImfIDManifest.cpp ──────────────────────────
...
EOS
    ...
    # ── compile harness + ImfIDManifest.cpp directly with UBSan ─────────────
    ...
    clang++ \
      ...
      "$SRC/Iex/IexBaseExc.cpp" \
      "$SRC/Iex/IexThrowErrnoExc.cpp" \
...
echo ""
echo "Expected: runtime error: shift exponent 70 is too large for 64-bit type 'uint64_t'"
echo "Source:   ImfIDManifest.cpp:122, commit c13e0e1 (2026-04-16)"
