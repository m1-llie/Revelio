...
* `detect.py` - Detect memory-safety vulnerabilities using Docker sandbox. Supports:
  ...
  - `--pipeline <mode>` - Pipeline mode: `file` (single file hypothesis), `project` (parallel hypotheses only), `detect` (full pipeline, default), `scan_filter` (multi-pass scan + filter), `scan_filter_detect` (scan_filter then PoV + report)
  ...
  - `--max-pov-attempts <n>` - Max validate tool calls per hypothesis (default: 3)
  - `--filter-model` - Model for scan_filter Stage 3 sub-agent verification
  - `--pov-model` - Model for PoV builder/reporter agents
  - `--hypotheses-file` - Load pre-generated hypotheses, skip scan stage

* `scan_and_filter.py` - Standalone scan-and-filter CLI for local repos (without Docker-based PoV generation). Runs multi-pass LLM analysis, classification/dedup, and Docker sub-agent filtering on individual source files.

* `validate_if_target_singleAgent.py` - Validate a PoV against both ARVO versions:
  - Pulls `-fix` image if not cached locally
  - Tests PoV on both `-vul` and `-fix` versions
  - Passes if: crashes on vul AND no crash on fix
  - Usage: `python -m revelio.run.validate_if_target_singleAgent --run-dir output/arvo-xxx/`

* `validate_if_target_multiAgent.py` - Validate a PoV from multi-agent runs:
  - Uses `manifest.json` to resolve the ARVO image (or `--arvo-image`)
  - Supports per-hypothesis PoVs (e.g., `hypothesis_H01/pov_H01`)
  - Usage:
    - `python -m revelio.run.validate_if_target_multiAgent --run-dir output/arvo-xxx/ --pov output/arvo-xxx/hypothesis_H01/pov_H01`

* `clean_arvo.py` - Prepare ARVO images for zero-day detection:
  - Removes pre-existing PoVs, crashers, and seed corpus
  - Creates new image tagged with `-clean` suffix
  - No rebuild needed (harness binaries preserved)
  - Usage: `python -m revelio.run.clean_arvo --image n132/arvo:14935-vul`
