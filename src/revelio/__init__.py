...
- Version numbering
- Protocols for the core components of revelio.
...
from revelio.utils.log import logger

package_dir = Path(__file__).resolve().parent

# Try to load .env from project root first, then fall back to global config
project_root = package_dir.parent.parent  # Go up from src/revelio/ to project root
project_env_file = project_root / ".env"
global_config_dir = Path(os.getenv("GLOBAL_CONFIG_DIR") or user_config_dir("revelio"))
...
if project_env_file.exists():
    if not os.getenv("SILENT_STARTUP"):
        Console().print(
            f"👋 This is [bold green]revelio[/bold green] version [bold green]{__version__}[/bold green].\n"
